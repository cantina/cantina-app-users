...
// Wait for start so we don't load cantina-web too early
app.hook('start').add(function (done) {
  ...
  app.auth.logIn = function authLogIn (user, req, res, next) {
  ...
  app.auth.logOut = function authLogOut (req, cb) {
    var user = req.user;
    if (!user) {
  ...
  app.auth.killAllSessions = function authKillAllSessions (user, cb) {
  ...
  app.auth.killSession = function authKillSession (user, session_id, cb) {
